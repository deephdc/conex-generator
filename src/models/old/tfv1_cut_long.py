...
# split data
condition = np.logical_and(label[:,1] > (3.4)*1e9, label[:,1] < (6.567)*1e9)
dropindex = np.where(condition)[0]
keepindex = np.where(np.logical_not(condition))[0]

dropdata = data[dropindex,:,:]
droplabel = label[dropindex,:]
data = data[keepindex,:,:]
label = label[keepindex,:]
...
with tf.Session() as sess:
    ...
    for x_batch, y_batch, n_batch, curstep, numsteps in yieldbatch(dropdata, droplabel, len(dropdata)):
        log.info("generating test set 2")
        starttime = timeit.default_timer()
        xout = sess.run(generatoroutput,
            feed_dict = {
                yin: droplabel,
        ...
        np.save("/cr/users/koepke/data/network/conex-generator/models/gan/run02/99/gdata_drop.npy", xout, fix_imports=False)
        np.save("/cr/users/koepke/data/network/conex-generator/models/gan/run02/99/label_drop.npy", droplabel, fix_imports=False)
        np.save("/cr/users/koepke/data/network/conex-generator/models/gan/run02/99/rdata_drop.npy", dropdata, fix_imports=False)
        runtime = timeit.default_timer() - starttime
        log.info(f"runtime: {runtime} seconds")
        break
